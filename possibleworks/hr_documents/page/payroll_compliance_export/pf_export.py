...
from possibleworks.hr_documents.page.payroll_compliance_export.constants import (
	MAPPED_FIELDS,
	MAX_PERIOD_MONTHS,
	NA,
)
...
def _get_salary_slips_with_components(company, start_date, end_date):
	"""Fetch submitted Salary Slips for `company` within [start_date, end_date],
	plus their earnings/deductions and each employee's UAN number — exactly
	three bulk queries total, regardless of employee count (no N+1)."""
	...
	uan_by_employee = {
		e.name: e.custom_uan_number
		for e in frappe.get_all(
			"Employee", filters={"name": ["in", employee_ids]}, fields=["name", "custom_uan_number"]
		)
	}

	for s in slips:
		s["components"] = components_by_slip.get(s.name, {})
		s["uan_number"] = uan_by_employee.get(s.employee)
...
def build_pf_rows(company, start_date, end_date, mapping):
	...
	{field_name: salary_component_name} for each field in MAPPED_FIELDS
	...
	"""
	slips = _get_salary_slips_with_components(company, start_date, end_date)
